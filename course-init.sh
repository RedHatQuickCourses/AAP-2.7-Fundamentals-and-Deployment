...
execute_init()
{
	...
	sed -i "s/AAP-2.7-Fundamentals-and-Deployment/${REPONAME}/g" *.* > /dev/null 2>&1
	sed -i "s/AAP-2.7-Fundamentals-and-Deployment/${REPONAME}/g" supplemental-ui/partials/header-content.hbs > /dev/null 2>&1
	echo -e "done"
